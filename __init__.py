...
The second layer also correctly handles partial execution and cached-nodes-only scenarios.
"""
...
def _short_name(path_str):
    """Extract 'geometrypack' from '.../geometrypack-nodes' or '.../geometrypack'."""
...
def _get_env_names():
    """Discover environment names using three independent methods.

    1. Proxy class closures — reads env_dir from comfy-env proxy classes
    2. Live worker pool — extracts keys from _WORKER_POOL
    3. Filesystem scan — scans comfy-env's envs/ directories

    Union of all three ensures we never miss an env.
    """
    names = set()
    global _mapping_cache

    # --- Method 1: proxy class closures ---
    ...
    # Skip pixi workspace env (no comfy-env worker)
...
def _kill_envs_by_name(target_names):
    """Kill workers for all given short env names (e.g. 'geometrypack', 'sharp')."""
    try:
...
async def _route_kill_all(request):
    try:
        import comfy_env.isolation.wrap as _wrap
...
# ---------------------------------------------------------------------------
# Manual Kill Node — trigger-based per-environment worker termination
# ---------------------------------------------------------------------------
